...
// Data structure
const CUSTOMERS = 5;
const FARMS_PER_CUSTOMER = 3;
const HOUSES_PER_FARM = 3;
...
async function generateLabData(structure, startDate) {
  ...
  for (const customer of structure) {
    for (const farm of customer.farms) {
      // Generate 2-5 lab readings per farm per day
      const readingsPerDay = faker.number.int({ min: 2, max: 5 });
...
async function generateSweepData(structure, startDate) {
  ...
  for (const customer of structure) {
    for (const farm of customer.farms) {
      for (const house of farm.houses) {
        // Generate 1-3 sweep readings per house per day
        const sweepsPerDay = faker.number.int({ min: 1, max: 3 });
...
async function generateDataIngestionLogs(sensorCount, labCount, sweepCount) {
  ...
  for (let i = 0; i < Math.ceil(sensorCount / 1000); i++) {
    const log = {
      source: faker.helpers.arrayElement(sources),
      dataType: 'sensor',
      recordCount: Math.min(1000, sensorCount - (i * 1000)),
    ...
    await postDataIngestionLog(log);
    totalLogs++;
  }
  
  // Generate logs for lab data
  for (let i = 0; i < Math.ceil(labCount / 100); i++) {
    const log = {
      source: faker.helpers.arrayElement(sources),
      dataType: 'lab',
      recordCount: Math.min(100, labCount - (i * 100)),
      status: faker.helpers.arrayElement(statuses),
      errorMessage: faker.helpers.arrayElement(statuses) === 'error' ? faker.lorem.sentence() : undefined,
      metadata: {
        batchId: `lab_batch_${i}`,
  ...
  for (let i = 0; i < Math.ceil(sweepCount / 50); i++) {
    const log = {
      source: faker.helpers.arrayElement(sources),
      dataType: 'sweep',
      recordCount: Math.min(50, sweepCount - (i * 50)),
      status: faker.helpers.arrayElement(statuses),
      errorMessage: faker.helpers.arrayElement(statuses) === 'error' ? faker.lorem.sentence() : undefined,
      metadata: {
        batchId: `sweep_batch_${i}`,
...
async function main() {
  ...
  startDate.setDate(startDate.getDate() - DAYS); // Start 3 days ago
