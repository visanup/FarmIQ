...
/**
 * EnvFactorService with Kafka pub-sub integration
 * Note: The EnvironmentalFactor entity defines id, customer_id, farm_id,
 * ventilation_rate, note, measurement_date, effective_start, effective_end,
 * and created_at only. No batch_id, updated_at columns exist in the model.
 */
...
export class EnvFactorService {
  ...
  async create(data: Partial<EnvironmentalFactor>): Promise<EnvironmentalFactor> {
    ...
    await publishEvent(
      topics.ENVIRONMENTAL_FACTORS_CREATED,
      {
        id: saved.id,
        customer_id: saved.customer_id,
        farm_id: saved.farm_id,
        ventilation_rate: saved.ventilation_rate,
        note: saved.note,
  ...
  async update(id: number, data: Partial<EnvironmentalFactor>): Promise<EnvironmentalFactor | null> {
    ...
    if (updated) {
      ...
      await publishEvent(
        topics.ENVIRONMENTAL_FACTORS_UPDATED,
        {
          id: updated.id,
          changed_fields: data,
  ...
  async delete(id: number): Promise<boolean> {
    ...
    if (success) {
      ...
      await publishEvent(
        topics.ENVIRONMENTAL_FACTORS_DELETED,
        {
          id: existing.id,
