...
/**
 * HousingConditionService with Kafka pub-sub integration
 * Note: The HousingCondition entity defines id, customer_id, farm_id,
 * flooring_humidity, animal_density, area, effective_start, effective_end,
 * and created_at only. No batch_id or updated_at columns exist in the model.
 */
...
export class HousingConditionService {
  ...
  async create(data: Partial<HousingCondition>): Promise<HousingCondition> {
    ...
    const saved = await this.repo.save(hc);

    // Publish created event
    await publishEvent(
      topics.HOUSING_CONDITIONS_CREATED,
      {
        id: saved.id,
        customer_id: saved.customer_id,
        farm_id: saved.farm_id,
        flooring_humidity: saved.flooring_humidity,
        animal_density: saved.animal_density,
        area: saved.area,
  ...
  async update(id: number, data: Partial<HousingCondition>): Promise<HousingCondition | null> {
    ...
    if (updated) {
      ...
      await publishEvent(
        topics.HOUSING_CONDITIONS_UPDATED,
        {
          id: updated.id,
          changed_fields: data,
  ...
  async delete(id: number): Promise<boolean> {
    ...
    if (success) {
      ...
      await publishEvent(
        topics.HOUSING_CONDITIONS_DELETED,
        {
          id: existing.id,
