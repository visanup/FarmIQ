...
import {
  ...
  Router as StationIcon, Search as SearchIcon, Refresh as RefreshIcon
...
const StationsPage: React.FC = () => {
    ...
    const loadStations = async () => {
        try {
            setLoading(true);
            setError(null);
            const data = await masterServiceClient.getStations();
            setStations(data);
    ...
    const handleMenuClick = (event: React.MouseEvent<HTMLElement>, item: Station) => {
    ...
    const filteredStations = stations.filter(item =>
        ...
        item.zoneId?.toLowerCase().includes(searchTerm.toLowerCase()) ||
    ...
    const handleRefresh = () => loadStations();
          ...
          <Fade in timeout={800}>
            <Box sx={{ p: 3, pb: 0 }}>
              <Card sx={{ p: 3, borderRadius: 4, boxShadow: '0 8px 32px rgba(0,0,0,0.1)', position: 'relative' }}>
                <Box sx={{ position: 'absolute', top: 0, left: 0, right: 0, height: 4, background: 'linear-gradient(90deg, #9C27B0 0%, #E91E63 100%)' }} />
                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                    <Avatar sx={{ width: 56, height: 56, background: 'linear-gradient(135deg, #9C27B0 0%, #E91E63 100%)', boxShadow: '0 4px 15px rgba(156,39,176,0.4)' }}>
                      <StationIcon sx={{ fontSize: 28 }} />
                    </Avatar>
                    <Box>
                      <Typography variant="h3" sx={{ fontWeight: 800, mb: 1, background: 'linear-gradient(135deg, #9C27B0 0%, #E91E63 100%)', backgroundClip: 'text', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>จัดการสถานี</Typography>
                      <Typography variant="h6" sx={{ color: 'text.secondary' }}>จัดการสถานีภายในโซนของคุณ</Typography>
                    </Box>
                  </Box>
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                    ...
                    <Button variant="contained" startIcon={<AddIcon />} onClick={() => handleOpenDialog()}>เพิ่มสถานีใหม่</Button>
                  </Box>
                </Box>
              </Card>
            </Box>
                      ...
                      {currentStations.length > 0 ? currentStations.map((item, index) => (
                        <Zoom in timeout={600 + (index * 100)} key={item.id}>
                          <TableRow hover>
                            <TableCell>
                              <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                                <Avatar sx={{ background: 'linear-gradient(135deg, #9C27B0 0%, #E91E63 100%)' }}><StationIcon /></Avatar>
                                <Typography sx={{ fontWeight: 600 }}>{item.name}</Typography>
                              </Box>
                            </TableCell>
                            <TableCell>{safeRenderValue(item.zoneId)}</TableCell>
                        ...
                        <TableRow>
                          <TableCell colSpan={5} align="center" sx={{ py: 8 }}>
                            <NoData message="ไม่พบข้อมูลสถานี" />
                          </TableCell>
                        </TableRow>
                      )}
                    </TableBody>
          ...
          {totalPages > 1 && (
              ...
              <Box sx={{ display: 'flex', justifyContent: 'center', mb: 4 }}>
                <Card sx={{ p: 2, borderRadius: 4 }}>
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 3 }}>
                    <Typography variant="body2" color="text.secondary">แสดง {startIndex + 1}-{Math.min(endIndex, filteredStations.length)} จาก {filteredStations.length} รายการ</Typography>
            ...
            {openDialog && (
                <Dialog open={openDialog} onClose={handleCloseDialog} maxWidth="sm" fullWidth>
                    <DialogTitle sx={{ background: 'linear-gradient(135deg, #9C27B0 0%, #E91E63 100%)', color: '#fff' }}>{editingItem ? 'แก้ไขสถานี' : 'เพิ่มสถานีใหม่'}</DialogTitle>
                    <DialogContent>
                        <Grid container spacing={2} sx={{ pt: 1 }}>
                            <Grid item xs={12}>
                                <TextField label="ชื่อสถานี" defaultValue={editingItem?.name || ''} fullWidth />
                            </Grid>
                            <Grid item xs={12} sm={6}>
                                <TextField label="โซน" defaultValue={editingItem?.zoneId || ''} fullWidth />
                            </Grid>
                            <Grid item xs={12} sm={6}>
