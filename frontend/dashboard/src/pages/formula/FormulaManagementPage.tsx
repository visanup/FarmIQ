...
import {
  ...
  Science as FormulaIcon, Search as SearchIcon, Refresh as RefreshIcon
} from '@mui/icons-material';
import { DashboardLayout } from '../../components/layout/DashboardLayout';
import NoData from '../../components/common/NoData';
import { masterServiceClient } from '../../services/api';
import { Formula, AnimalType } from '../../types/api';
...
const FormulaManagementPage: React.FC = () => {
    ...
    const loadData = async () => {
        try {
            setLoading(true);
            setError(null);
            const [formulasData, animalTypesData] = await Promise.all([
                masterServiceClient.getFormulas(),
                masterServiceClient.getAnimalTypes()
            ]);
    ...
    const getAnimalTypeName = (animalTypeId: string) => {
        const animalType = animalTypes.find(at => at.id === animalTypeId);
        return animalType ? animalType.name : animalTypeId;
    };

    const filteredFormulas = formulas.filter(item =>
        item.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
        getAnimalTypeName(item.animalTypeId).toLowerCase().includes(searchTerm.toLowerCase()) ||
        ...
        <Box sx={{ background: 'linear-gradient(135deg, #f8fafc 0%, #e2e8f0 100%)', minHeight: '100vh' }}>
          ...
          <Fade in timeout={800}>
            <Box sx={{ p: 3, pb: 0 }}>
              <Card sx={{ p: 3, borderRadius: 4, boxShadow: '0 8px 32px rgba(0,0,0,0.1)', position: 'relative' }}>
                <Box sx={{ position: 'absolute', top: 0, left: 0, right: 0, height: 4, background: 'linear-gradient(90deg, #9C27B0 0%, #E1BEE7 100%)' }} />
                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                    <Avatar sx={{ width: 56, height: 56, background: 'linear-gradient(135deg, #9C27B0 0%, #E1BEE7 100%)', boxShadow: '0 4px 15px rgba(156,39,176,0.4)' }}>
                      <FormulaIcon sx={{ fontSize: 28 }} />
                    </Avatar>
                    <Box>
                      <Typography variant="h3" sx={{ fontWeight: 800, mb: 1, background: 'linear-gradient(135deg, #9C27B0 0%, #E1BEE7 100%)', backgroundClip: 'text', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>จัดการสูตรอาหาร</Typography>
                      <Typography variant="h6" sx={{ color: 'text.secondary' }}>จัดการสูตรอาหารสำหรับปศุสัตว์</Typography>
                    </Box>
                  </Box>
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                    ...
                    <Button variant="contained" startIcon={<AddIcon />} onClick={() => handleOpenDialog()}>เพิ่มสูตรอาหาร</Button>
                  </Box>
                </Box>
              </Card>
            </Box>
                ...
                <TableContainer>
                  <Table>
                    <TableHead sx={{ background: 'linear-gradient(135deg, #f8fafc 0%, #e2e8f0 100%)' }}>
                      <TableRow>
                        <TableCell>ชื่อสูตรอาหาร</TableCell>
                    ...
                    <TableBody>
                      {currentFormulas.length > 0 ? currentFormulas.map((item, index) => (
                        <Zoom in timeout={600 + (index * 100)} key={item.id}>
                          <TableRow hover>
                            <TableCell>
                              <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                                <Avatar sx={{ background: 'linear-gradient(135deg, #9C27B0 0%, #E1BEE7 100%)' }}><FormulaIcon /></Avatar>
                                <Typography sx={{ fontWeight: 600 }}>{item.name}</Typography>
                              </Box>
                            </TableCell>
                            <TableCell><Chip label={getAnimalTypeName(item.animalTypeId)} size="small" color="primary" /></TableCell>
                        ...
                        <TableRow>
                          <TableCell colSpan={5} align="center" sx={{ py: 8 }}>
                            <NoData message="ไม่พบข้อมูลสูตรอาหาร" />
                          </TableCell>
                        </TableRow>
                      )}
                    </TableBody>
              ...
              <Box sx={{ display: 'flex', justifyContent: 'center', mb: 4 }}>
                <Card sx={{ p: 2, borderRadius: 4 }}>
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 3 }}>
                    <Typography variant="body2" color="text.secondary">แสดง {startIndex + 1}-{Math.min(endIndex, filteredFormulas.length)} จาก {filteredFormulas.length} รายการ</Typography>
          ...
          {openDialog && (
            <Dialog open={openDialog} onClose={handleCloseDialog} maxWidth="sm" fullWidth>
              <DialogTitle sx={{ background: 'linear-gradient(135deg, #9C27B0 0%, #E1BEE7 100%)', color: 'white' }}>
                {editingItem ? 'แก้ไขสูตรอาหาร' : 'เพิ่มสูตรอาหารใหม่'}
              </DialogTitle>
              <DialogContent>
                <Grid container spacing={2} sx={{ pt: 1 }}>
                  <Grid item xs={12}>
                    <TextField margin="dense" label="ชื่อสูตรอาหาร" defaultValue={editingItem?.name || ''} fullWidth />
                  </Grid>
                  <Grid item xs={12}>
                    <TextField 
                      margin="dense" 
                      label="ประเภทสัตว์" 
                      select
                      defaultValue={editingItem?.animalTypeId || ''} 
                      fullWidth
