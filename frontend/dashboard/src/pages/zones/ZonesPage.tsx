...
import { Pagination, Card, CardContent, Fade, Zoom, useTheme } from '@mui/material';
...
const ZonesPage: React.FC = () => {
    ...
    const filteredZones = zones.filter(item =>
        ...
        item.farmId?.toLowerCase().includes(searchTerm.toLowerCase())
    );
          ...
          <Fade in timeout={800}>
            <Box sx={{ p: 3, pb: 0 }}>
              <Card sx={{ p: 3, borderRadius: 4, boxShadow: '0 8px 32px rgba(0,0,0,0.1)', position: 'relative' }}>
                <Box sx={{ position: 'absolute', top: 0, left: 0, right: 0, height: 4, background: 'linear-gradient(90deg, #3F51B5 0%, #2196F3 100%)' }} />
                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                    <Avatar sx={{ width: 56, height: 56, background: 'linear-gradient(135deg, #3F51B5 0%, #2196F3 100%)', boxShadow: '0 4px 15px rgba(33,150,243,0.4)' }}>
                      <ZoneIcon sx={{ fontSize: 28 }} />
                    </Avatar>
                    <Box>
                      <Typography variant="h3" sx={{ fontWeight: 800, mb: 1, background: 'linear-gradient(135deg, #3F51B5 0%, #2196F3 100%)', backgroundClip: 'text', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>จัดการโซน</Typography>
                      <Typography variant="h6" sx={{ color: 'text.secondary' }}>จัดการโซนภายในโรงเรือนของคุณ</Typography>
                    </Box>
                  </Box>
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                    <Typography variant="body2" sx={{ color: 'text.secondary' }}>อัปเดตล่าสุด: {lastUpdate.toLocaleString('th-TH')}</Typography>
                    <Button variant="outlined" onClick={handleRefresh}>รีเฟรช</Button>
                    <Button variant="contained" startIcon={<AddIcon />} onClick={() => handleOpenDialog()}>เพิ่มโซนใหม่</Button>
                  </Box>
                </Box>
              </Card>
            </Box>
              ...
              <Card sx={{ borderRadius: 4 }}>
                <TableContainer>
                  <Table>
                    ...
                    <TableBody>
                      {currentZones.length > 0 ? currentZones.map((item, index) => (
                        <Zoom in timeout={600 + (index * 100)} key={item.id}>
                          <TableRow hover>
                            <TableCell>
                              <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                                <Avatar sx={{ background: 'linear-gradient(135deg, #3F51B5 0%, #2196F3 100%)' }}><ZoneIcon /></Avatar>
          ...
          {totalPages > 1 && (
              ...
              <Box sx={{ display: 'flex', justifyContent: 'center', mb: 4 }}>
                <Card sx={{ p: 2, borderRadius: 4 }}>
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 3 }}>
                    <Typography variant="body2" color="text.secondary">แสดง {startIndex + 1}-{Math.min(endIndex, filteredZones.length)} จาก {filteredZones.length} รายการ</Typography>
            ...
            {openDialog && (
                <Dialog open={openDialog} onClose={handleCloseDialog} maxWidth="sm" fullWidth>
                    <DialogTitle sx={{ background: 'linear-gradient(135deg, #3F51B5 0%, #2196F3 100%)', color: '#fff' }}>{editingItem ? 'แก้ไขโซน' : 'เพิ่มโซนใหม่'}</DialogTitle>
                    <DialogContent>
                        <Grid container spacing={2} sx={{ pt: 1 }}>
                            <Grid item xs={12}>
                                <TextField label="ชื่อโซน" defaultValue={editingItem?.name || ''} fullWidth />
                            </Grid>
